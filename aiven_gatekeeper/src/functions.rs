use crate::{
    ...
    roles::{is_elevated, is_local_user_id_change},
};
use pgrx::pg_sys;
use std::ffi::CString;

const RESERVED_FUNCTION_NAMES: [&str; 11] = [
    "pg_read_file",
...
const RESERVED_SO_PATHS: [&str; 4] = ["file_fdw", "file_fdw.so", "plperl", "plperl.so"];
...
pub fn is_function_language_allowed(in_strict_mode: bool) -> Result<bool, &'static str> {
    if in_strict_mode || is_elevated() || is_security_restricted() || is_local_user_id_change() {
...
pub fn is_reserved_internal_function_oid(
    func_oid: pg_sys::Oid,
    min_reserved_oid: u32,
    max_reserved_oid: u32,
    reserved_function_oids: &Vec<pg_sys::Oid>,
...
fn get_reserved_func_name_from_oid(func_oid: pg_sys::Oid) -> Result<&'static str, &'static str> {
    for r_func_name in RESERVED_FUNCTION_NAMES {
        ...
        let oid: pg_sys::Oid =
            unsafe { pg_sys::fmgr_internal_function(c_str.as_ptr() as *const i8) };
        if oid == func_oid {
