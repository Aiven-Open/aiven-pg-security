...
fn copy_stmt_checks(stmt: *mut pg_sys::Node) {
    let copy_stmt: PgBox<pg_sys::CopyStmt> =
        unsafe { PgBox::from_pg(stmt as *mut pg_sys::CopyStmt) };
...
fn create_extension_checks(stmt: *mut pg_sys::Node) {
    ...
    let create_ext_stmt: PgBox<pg_sys::CreateExtensionStmt> =
        unsafe { PgBox::from_pg(stmt as *mut pg_sys::CreateExtensionStmt) };
    if let Ok(extname) = unsafe { std::ffi::CStr::from_ptr(create_ext_stmt.extname).to_str() } {
        // check if disallowed extension
        if extname == "file_fdw" {
            // error and abort the current transaction if disallowed extension
    ...
    for opt_raw in options_lst.iter_ptr() {
        ...
        if let Ok(option_name) = util::to_cstr(option.defname) {
            ...
            if let Ok(role_name) = util::to_cstr(create_role_stmt.role) {
                if is_allowed_superuser_role(
                    role_name,
                    GUC_RESERVED_SU_ROLES.get().unwrap().to_str().unwrap(),
                ) == false
                {
...
fn alter_role_checks(stmt: *mut pg_sys::Node) {
    ...
    let options_lst: pgrx::PgList<pg_sys::DefElem> =
        unsafe { pgrx::PgList::from_pg(alter_role_stmt.options) };
    for opt_raw in options_lst.iter_ptr() {
        option = unsafe { PgBox::from_pg(opt_raw as *mut pg_sys::DefElem) };
        if let Ok(option_name) = util::to_cstr(option.defname) {
            // check if role is allowed to be a superuser, is in GUC_RESERVED_SU_ROLES
...
fn grant_role_checks(stmt: *mut pg_sys::Node) {
    let grant_role_stmt: PgBox<pg_sys::GrantRoleStmt> =
        unsafe { PgBox::from_pg(stmt as *mut pg_sys::GrantRoleStmt) };
    let lst: pgrx::PgList<pg_sys::AccessPriv> =
        unsafe { pgrx::PgList::from_pg(grant_role_stmt.granted_roles) };
    for granted_role in lst.iter_ptr() {
        let access_privilege: PgBox<pg_sys::AccessPriv> =
            unsafe { PgBox::from_pg(granted_role as *mut pg_sys::AccessPriv) };
        //let priv_name = std::ffi::CStr::from_ptr(access_privilege.priv_name).to_string_lossy().into_owned();
        let roloid: pg_sys::Oid =
            unsafe { pg_sys::get_role_oid(access_privilege.priv_name, false) };
...
fn function_create_checks(stmt: *mut pg_sys::Node) {
    ...
    let options_lst: pgrx::PgList<pg_sys::DefElem> =
        unsafe { pgrx::PgList::from_pg(create_func_stmt.options) };

    // check return_type, this is the cheapest check, so do it first
    let type_name: PgBox<pg_sys::TypeName> =
        unsafe { PgBox::from_pg(create_func_stmt.returnType as *mut pg_sys::TypeName) };
    ...
    for opt_raw in options_lst.iter_ptr() {
        ...
        if let Ok(option_name) = util::to_cstr(option.defname) {
            ...
            if option_name == "language" {
                if let Ok(lang_name) =
                    util::to_cstr(unsafe { pg_sys::defGetString(option.as_ptr()) })
                {
    ...
    if check_body {
        ...
        if let Ok(sql_body) = util::to_cstr(sql_body_ptr) {
            ...
            // in the case of C functions, we want to check the path it is being created from
...
extern "C" fn object_access_hook(
    ...
    if is_agent_enabled() && access == OAT_FUNCTION_EXECUTE {
        ...
        if let Ok(fname) =
            is_reserved_internal_function_oid(object_id, min_oid, max_oid, &reserved_oids)
        {
            if is_strict_mode_enabled()
                || is_elevated()
                || is_security_restricted()
            ...
            if !unsafe { superuser() } {
                pg_sys::error!(
                    "using builtin function {} is not allowed by non-superusers",
                    fname
                );
            }
        }
...
fn get_cached_builtin_oids() -> (u32, u32, &'static mut Vec<pg_sys::Oid>) {
    unsafe {
        INIT_RESERVED_OIDS.call_once(|| {
            ...
            (MIN_RESERVED_OID, MAX_RESERVED_OID) =
                resolve_internal_func_oids(&mut RESERVED_BUILTIN_OIDS);
        });
        (
            MIN_RESERVED_OID,
            MAX_RESERVED_OID,
            &mut RESERVED_BUILTIN_OIDS,
        )
    }
}
...
pub extern "C" fn _PG_init() {
    ...
    GucRegistry::define_string_guc(
        ...
        GucFlags::SUPERUSER_ONLY
            | GucFlags::DISALLOW_IN_AUTO_FILE
